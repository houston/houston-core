...
class DeploysControllerTest < ActionController::TestCase
  ...
  test "should not require a logged in user" do
    mock(Deploy).create!({project: @project, environment_name: @environment, commit: "hi", deployer: nil})
    post :create, {project_id: @project.slug, environment: @environment, commit: "hi"}
  end
  
  
  test "should work with a Heroku-style deploy hook" do
    mock(Deploy).create!({project: @project, environment_name: @environment, commit: "hi", deployer: "deployer@heroku.com"})
  ...
  test "should record the deployer if it is supplied" do
    mock(Deploy).create!({project: @project, environment_name: @environment, commit: "hi", deployer: "Bob Lail <bob.lail@cph.org>"})
